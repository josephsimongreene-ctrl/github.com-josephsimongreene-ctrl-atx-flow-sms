...
// Twilio calls this when call comes in - lets it ring then hang up
app.post("/voice", (req, res) => {
  const twiml = new twilio.twiml.VoiceResponse();
  twiml.pause({ length: 20 }); // Let it ring for 20 seconds
  res.type("text/xml");
  res.send(twiml.toString());
});

// Twilio calls this when call status changes to no-answer/busy/failed
app.post("/missed-call", async (req, res) => {
  const callerNumber = req.body.From;
  const callStatus   = req.body.CallStatus;

  console.log(`📞 Call from ${callerNumber} — status: ${callStatus}`);
